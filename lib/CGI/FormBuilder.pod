...
What will be the ID and name of a submit button in the case if the
element of the passed array is itself a (two-element) array or an (one-key)
hash, is undefined and it may change in a future version of FormBuilder.
