...
=item disable_enter => 0 | 1

If set to 0 (default), Enter key pressed in a text field submits the form.
If set to 1, Enter key pressed in a text field does not submit the
form.
...
If an element of the passed array is itself a (two-element) array
or an (one-key) hash then the first value of the array or the key of the hash is
taken as the ID returned by C<submitted()> function and the second value
is taken as the label for the button. WARNING: This works properly only
when JavaScript is enabled!

In this case the first element of the two element array (or the key of the
one-key hash) becomes the ID of the submit button. The old behavior when to
submit buttons are automatically assigned IDs C<_submit>, C<_submit_2> etc.
